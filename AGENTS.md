...
- 只有在用户明确要求时才创建提交。
- 起草提交信息前，先检查已暂存 diff，按实际改动确定主要交付结果、影响范围和验证情况；不要直接沿用任务标题或开发计划名称。
- 提交信息使用 Conventional Commits：`type(scope): summary`；无合适 scope 时省略。
- 常用类型：`feat`、`fix`、`docs`、`test`、`refactor`、`build`、`ci`、`chore`。
- summary 使用英文祈使语气，简短明确，不加句号；优先描述用户或项目新获得的行为和能力。
- 避免用 `vertical slice`、`phase 1`、`groundwork` 等计划阶段词作为 summary 的核心，也不要在同时交付完整功能时只突出某个内部实现步骤。
- 一次提交包含多个方面时，summary 描述主要结果，其余关键行为、架构边界和兼容性影响放在正文中。
- 非简单变更必须有正文，简要说明改动目的和关键行为，并只列出实际执行过的验证命令；未执行的检查应明确标为未执行，不得写成已验证。
- 提交正文必须使用真实的 UTF-8 换行。禁止把 `\n`、`\r\n` 或其他 shell 转义序列写成正文中的字面字符。
- 创建提交后、推送前，必须运行 `git show --format=fuller --stat HEAD` 检查标题、正文、换行和改动摘要是否准确；发现问题时先修正提交信息，再进行推送。
