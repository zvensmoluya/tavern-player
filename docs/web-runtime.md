...
- 正文排版基线：`src/message.css` 由主壳与 static 帧共用（`test/browser/render.test.mjs` 断言含 Markdown 表格的普通正文在 static 帧内与壳文档字体、行距、单元格边框一致），作者自己的样式仍然覆盖基线。
