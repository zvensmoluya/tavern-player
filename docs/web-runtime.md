...
更新：2026-09-15。运行配置为 `player-web-1`。这是声明范围内的兼容宿主，不是完整 SillyTavern 或 Tavern Helper。默认新建对话使用网页模式；已有记录缺省解释为旧 Native 模式，切换模式必须新建对话。
...
普通 Markdown/HTML 经 DOMPurify 清理，禁止作者脚本和事件属性。包含完整 `<body>…</body>` 的围栏代码块（标记为 html 时也接受省略 `</body>` 的合法 HTML）在消息 COMPLETE 后作为作者页面装载；其他完整代码块保留为代码。流式预览每 50 ms 至多一次；收到结束事件后继续接收用量，最终正文、状态与用量事务保存成功才发布 COMPLETE 并装载作者页面。

流式展示暂存未完整的 HTML 标签、属性引号、注释和 `<style>` / `<script>` 块，保留它们之前已可读的正文。尚未识别出 body 的未闭合 HTML 围栏也先占位，避免 head/CSS 源码随 token 闪出；普通编程代码和行内代码继续按 Markdown 显示。这里仅处理展示副本，不裁剪原始流或持久正文。已停止、中断和失败的页面段继续显示对应占位状态；COMPLETE 后移除生成状态，页面准备期间占位改为“正在加载交互界面…”，就绪才移除占位。

普通正文在同一 DOM 内比较节点与属性，纯尾部续写使用原文本节点追加数据，未变化的段落和前文选区保留。含表格、图片或内联 HTML 的段落在持久 static 帧中渲染；同一段、同一候选的内容变化通过 `static-update` 更新，清洗和 DOM 修补在该帧内进行，不重新设置 srcdoc 或创建网页。未变化的图片、输入节点及 details 展开状态保留；候选变化会创建新的静态文档。帧内沿用 `src/message.css` 正文排版基线，作者内联声明与 `<style>` 仍可覆盖它，作者样式不会进入可信外壳。已转为 static 的段在同一候选内继续使用该文档，避免投影暂时变为纯文字时来回切换。

从文字转为 static，或从占位转为作者页面时，新帧在原位置隐藏加载，旧文字或占位保持可见；最新 static 内容已应用并测量后才显示。准备期间只保留最新待发送静态内容，不阻塞后续消息渲染。显示采用 120 ms 淡入并遵从减少动态效果偏好，不做高度动画。作者页面仍只在 COMPLETE 后执行，源码或候选变化时先撤销旧页面身份再启动新页面；重建期间保留原实测高度，其他文字或思考变化、滚动及追加新消息不会重跑旧页面。

原生首次发送完整快照，后续发送变更消息、顺序与变更状态。首次显示最近 50 条，点击每次向前加载 50 条。标题、思考、正文段与状态分别更新；已挂载页面在历史被截断、切换对话或退出页面时释放。正文变化仍解析当前消息的完整 Markdown，并比较当前段的 DOM；这不是有界的增量解析器。新 CSS 生效、图片取得尺寸或完整交互页面出现仍可能改变最终布局，不保证任意作者 HTML 都能逐字呈现或完全没有重排。

消息区向上滚动即退出自动贴底，即使仍在底部 80 像素以内；折叠思考的流式更新同样尊重阅读位置。向下滚回底部附近或点击回底按钮后恢复跟随。
...
2026-09-15 补充视口事件乱序处理：帧测量使用已处理的视口高度记录底部，resize 完成前不让 scroll 覆盖阅读意图；位置未变化的迟到 scroll 也不取消已安排的贴底操作。桌面回归显式交错帧测量、resize 与 scroll，验证键盘缩小视口后仍能跟随，同时保留作者字段焦点期间暂停跟随的行为。
...
- 书级操作：新建的书先不参与编排，要由 `rebindCharWorldbooks` 或 `setWorldbookEnabled` 显式启用。`rebindCharWorldbooks` 在本次对话的书集合内同时决定参与项与顺序，不新增或删除书。这些作者接口的书集合与参与状态仍属于这场对话；玩家的独立全局书库由原生管理页维护，尚未映射到作者接口的注册表或绑定。这与上游“全局世界书目录 + 绑定”的结构不同。
...
可信外壳按消息对象变化处理行，正文按 display 文本及是否可展示最终内容复用解析结果；只有 reasoning、草稿或非 COMPLETE 状态之间的变化不重跑正文 Markdown，进入 COMPLETE 时重新判定此前缓冲的内容。原生桥在顺序未变时省略 `order`，外壳将变化字段与变化消息直接传给作者协调器；协调器在没有待确认写入时只复制变化内容，草稿增量保留历史消息数组。有待确认写入时仍保守重建 overlay，作者操作结果仍可携带完整权威快照，因此这还不是设计中的全链路 `player-bridge-2`。
...
- 流式 DOM 更新与状态文案：`test/browser/render.test.mjs` 在真实 Edge 上量富文本更新期间的文档高度与滚动位置；验证静态文档、表单焦点/选区、图片和 details 状态保留，纯文字续写保留前文节点及选区；还覆盖 CSS 缓冲、慢页面准备时保留占位且后续消息继续渲染、静态加载追上最新内容、更新路径清洗，以及作者页面源码/候选改变时按契约重建。`test/markdown.test.mjs` 按每个字符前缀验证 HTML 围栏源码不闪出，普通代码、链接和表格语义保留。
