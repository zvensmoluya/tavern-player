...
2026-09-15 已实施正文解析复用、作者协调器增量接收、轻量页面配置、正则缓存、按条 token 预算，以及 Room 分表存储与旧数据导入、ConversationSession 统一写入、原始流日志与最终事务。实际边界及验证见[性能改进记录](performance-improvements-20260915.md)和[会话存储与事务实现](conversation-storage.md)。以下是完整目标设计；有界展示窗口、完整原生变化集、挂起式 EJS 等尚未全部交付。
...
2026-09-15 已补充流式 DOM 更新：普通正文保留节点并追加尾部文字，static iframe 在原文档内清洗和修补，未完整 HTML/CSS 先缓冲；作者页面在 COMPLETE 后隐藏准备，再替换占位。正文变化仍解析整条 display，尚未达到增量解析或所有工作量有界，完整行为及限制见[网页运行契约](web-runtime.md)。
