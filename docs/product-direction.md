...
更新：2026-09-10。本页维护产品方向、稳定原则与当前能力边界；实现细节和参数只在架构文档维护。具体代码边界见[实现架构](architecture.md)，当前路线讨论见[HTML Surface 与免编译游玩](html-surface-discussion-20260909.md)。历次暂停、编译版本与旧阶段决定见[归档快照](archive/product-decisions-through-20260909.md)，不再作为并列的当前要求。
...
- 新建会话默认使用网页模式，已有会话保留原模式；切换通过新建完成。导入和 Shelf 接收不执行程序，进入网页会话后按 [player-web-1 契约](web-runtime.md)装载原文与资源，不要求模型编译。原生入口对每张卡可用：纯文字卡直接进原生，已安装 Native 适配沿用原入口且不再显示该说明；卡含作者脚本或世界书模板时，入口处说明这些内容不会在原生模式运行。
...
- 玩家对世界书的操作只发生在会话内，不编辑角色资产。提供三种操作：条目的启用／停用（两态）、整本书的参与方式（不参与／自动／必定生效三态）、以及条目正文编辑。编排字段（关键字、位置、深度、顺序、概率、分组、递归、计时效果）不向玩家开放：它们是作者的设计意图，改它们等于拆卡而不是定制。角色库入口保持只读，角色卡级别的世界书编辑不提供。
- “必定生效”是 Player 自己的产品能力，不是 ST 兼容语义：该书当前已启用的条目按常开处理并跳过概率，启停、互斥分组、预算、顺序与位置照常生效。ST 只有条目的常开标记与正文装饰器，没有按书强制激活的概念，因此该能力须记录为 Player 扩展。
- 正文编辑沿用既有投影约束：被登记为 EJS 模板的条目在正文改写后不再执行模板并给出诊断，改写为普通文字后按普通条目参与编排。被玩家改过的条目应在会话内显式标明。
- 玩家意图与剧情派生状态分开归属：条目启用、书级参与方式和正文修改属于会话级，不随消息候选切换回退；sticky / cooldown 等由剧情派生的运行状态继续跟随候选检查点，delay 根据该会话当前选中分支的消息数判断。
- 保留受支持的触发、排序、注入、预算和跨轮状态语义。高级正则、扫描格式和递归仍有明确边界，不能将功能覆盖视为整体语义等价；具体行为与证据见[实现架构](architecture.md)。
- World Book 定义随 Character 一起进入 Character Snapshot。
...
- 网页模式直接读取原脚本、消息页面和 EJS 入口，不使用模型材料筛选。Native 编译、安装与原生显示机制保留为可选增强。两条路线均只提供声明的宿主能力，不承诺任意扩展。
...
- Shelf 通过二维码传输单份原始 Character 或受支持的 Preset，接收端校验后走本地导入。独立 World Book 当前只识别类型，不执行导入；协议见[传输契约](reference/transfer-protocol-v1.md)。
