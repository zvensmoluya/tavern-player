...
实施状态更新：网页消息区与 `player-web-1` 原程序运行环境已接入应用，当前能力、限制和验证见 [网页运行契约](web-runtime.md)。下文保留实施前的讨论记录，其中“尚未实现”和待决定事项反映当时状态。
...
- 宿主能力面：探针已复核加载期与部分定向交互，见[运行时探针记录](archive/html-surface-probe-20260909.md)。观察到消息写入请求、事件后状态重读，以及绕过 API 的父页面 DOM 依赖；结果受模拟宿主、资源阻断和样本选择影响，不能当作完整接口清单或兼容性证明。
...
兼容层以固定版本的 Tavern Helper、MVU、EJS 公开契约与实现为依据，先明确其对 ST 的依赖，再对照 Player 已有能力。卡片只用于选取与验证具体场景，不从卡片调用统计推断完整接口语义。现有探针保留为辅助验证工具，暂停扩建，不作为 WebView 接入的前置工程。
