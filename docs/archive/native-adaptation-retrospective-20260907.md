...
> 同日后续：用户要求实际尝试引入 MVU 公共变量能力，已完成 [固定上游源码接入实验](mvu-integration-probe-20260907.md)。本页保留暂停时的复盘；实验未扩建旧固定能力契约，也未切换 Android 运行时。

> 后续已按用户选择增加 [QuickJS Kotlin 宿主与消息检查点接入](mvu-quickjs-integration-20260907.md)，不再停留于 Node 参照实验。普通聊天的自动启用仍未完成。
...
样本编号见 [community-samples.md](../reference/community-samples.md)。本轮以 C-04 为主，C-03 为对照，核查启用脚本、正则、变量规则及条件模板，并与实际生成的适配结果比较。
