...
> 最新讨论（2026-09-09）：基础游玩不依赖逐卡模型编译，以系统 WebView 为首选验证对象，复用 Player 核心并对齐酒馆宿主接口。[HTML Surface 与免编译游玩](../html-surface-discussion-20260909.md) 已整理收敛结论评审版：Player 保持会话所有权、Native Adaptation 为可选增强、QuickJS 默认保留、唯一业务提交路径；同步状态视图、资源 origin 隔离和 Surface 粒度仍待验证。当前只更新讨论记录，HTML 路径尚未实现。

> 既有实现（2026-09-08）：已按用户决定进入实现，`native-compiler-11` 接通 JS 动态投影、高层 Native Surface 和操作检查点。开放程序表达、收紧界面表达；通用组件树不是默认能力。实际接口与验证边界见 [首轮实现](native-script-surfaces-20260908.md)。下文早期版本和暂停决定保留为历史。
...
## 2026-09-09：图片属于角色卡的持久资源

- 图片等已取得的配套资源属于 Character 资产，由 App 持久保留，归属方式与角色内的世界书一致；“缓存”仅是此前对下载到本地的口头称呼，不表示可自动淘汰的临时数据。
- 当前优先准备本地资源，使已取得的图片可离线读取，不以复刻原界面或交互为前置目标。文件放在 App 内部私有存储，不进入系统相册。
- 原图按角色资产及现有会话引用管理生命周期，不纳入普通缓存清理或自动淘汰；缩略图、解码缓存和下载临时文件可以重建或清理。删除角色时须考虑仍引用资源的会话。
- 原始远程地址用于来源追溯与缺失时补取，不代替已保存的文件；图床失效不使已有资源失效。备份、迁移和导出需按角色资产整体考虑，不因资源来自下载就默认排除。具体备份与打包方案尚未定案。
- 已实现独立“角色资源”入口：本地扫描静态引用，用户点击准备后下载、校验和持久保存，支持暂停、重试与离线预览。编译器、模型与聊天图片绑定本轮不参与；下文“远程图片延期”保留为历史边界。详见[角色图片资源](character-image-resources-20260909.md)。

## 2026-09-08：保留可执行步骤并提前验证原程序

- `native-compiler-15` 按输入、计算／预览、状态写入、草稿、宿主动作拆解流程。缺少某个宿主动作不再作为放弃独立可用步骤的理由；人工步骤必须明确标注，不宣称自动执行或整流程等价。
- 条件拼接的草稿使用 JS Surface；固定模板约束不适用时不能直接判定“表单不支持”。运行时多项选择可用独立切换操作及有序私有列表表达，保留选择顺序和全选／清空；不伪装成单选。
- 开场选择信息只读暴露给脚本，用户先用已有候选控件选定开场，再填写该分支表单。候选仍拥有各自状态；不新增自动切换或自动发送接口。
- MVU Schema helper 的已知静态／字面量动态导入映射到本地固定模块，保留 await 顺序，不下载源码。编译结果在返回 Ready 前实际加载并初始化原 MVU 和原开场；失败保留已有适配。
